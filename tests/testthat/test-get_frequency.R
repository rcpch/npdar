...
test_that("NA responses are excluded from numerator and denominator", {
  expect_equal(sum(is.na(summary_long$response)), 0)
  expect_false("NA" %in% unique(summary_long$response))
...
test_that("get_frequency() handles a measure with all NA values", {
  ...
  summary_test <- get_frequency(data = df_allna,
                                measures = "q3_catq")
  # All NA means no rows survive the filter(!is.na(response)) step
...
test_that("q1_mcq_optionA is exactly 100/0 T/F for England and 0/100 for Wales", {
  expect_equal(summary_long |> filter(measure == "q1_mcq_optionA" & country == "England" & response == "TRUE") |> pull(percent), 1)
  expect_equal(summary_long |> filter(measure == "q1_mcq_optionA" & country == "England" & response == "FALSE") |> pull(percent), 0)
  expect_equal(summary_long |> filter(measure == "q1_mcq_optionA" & country == "Wales" & response == "TRUE") |> pull(percent), 0)
  expect_equal(summary_long |> filter(measure == "q1_mcq_optionA" & country == "Wales" & response == "FALSE") |> pull(percent), 1)
})
